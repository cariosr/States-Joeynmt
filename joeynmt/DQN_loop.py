...
class QManager(object):
    ...
    def Reward_bleu_diff(self, trg, hyp, show = False):
        ...
        trg_temp = torch.zeros(size=[1,len(trg[0])+1], dtype=torch.int)

        trg_temp[0,0] = self.bos_index

        trg_temp[0,1:] = trg

        trg = trg_temp

        #print('Entra con: ', trg, hyp)



        smooth = 0.001

        rew = np.zeros([len(hyp[0])+1])
